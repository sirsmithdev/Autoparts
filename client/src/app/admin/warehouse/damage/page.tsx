...
export default function RecordDamagePage() {
  ...
  const selectedBin = productBins.find((pb) => pb.bin.id === binId);
  const maxQty = selectedBin?.quantity ?? 0;

  const damageMutation = useMutation({
    mutationFn: () =>
      api("/api/store/admin/warehouse/damage", {
        method: "POST",
        body: JSON.stringify({ productId: selectedProduct!.id, binId, quantity, reason }),
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold">Record Damaged Stock</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Deduct damaged items from inventory</p>
      </div>
      ...
      <div className="border rounded-md bg-card p-5 space-y-4 max-w-lg">
        ...
        <div>
          ...
          <input
            ...
            className="w-full mt-1 border rounded-lg px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring"
            value={quantity}
            onChange={(e) => setQuantity(Math.min(parseInt(e.target.value) || 1, maxQty || 999))}
          />
        </div>
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            disabled={!selectedProduct || !binId || !reason.trim() || quantity < 1 || damageMutation.isPending}
            className="inline-flex items-center gap-2 px-4 py-2 bg-destructive text-destructive-foreground rounded-lg text-sm font-medium hover:bg-destructive/90 disabled:opacity-50 transition-colors"
          >
            {damageMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <AlertTriangle className="h-4 w-4" />}
            Record Damage
          </button>
        </div>
      </div>
